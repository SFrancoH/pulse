  ...
  return (
    <main id="pulse-venta-root" className="overflow-x-hidden bg-[#F2EDE4] pb-36 text-[#1A1A1A]">
        ...
        <div className="mx-auto flex max-w-[1100px] items-center justify-center gap-3 max-[932px]:flex-col">
          ...
          <div className="grid w-full max-w-3xl grid-cols-1 gap-2">
            <div className="flex w-full gap-2 max-[932px]:flex-col">
              <input value={busqueda} onChange={(e) => setBusqueda(e.target.value.replace(/\D/g, "").slice(0, 4))} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); buscarNumero(); } }} placeholder="Buscar número exacto... Ej: 5555" className="w-full rounded-md border border-[#E0D9CE] bg-[#F2EDE4] px-4 py-2.5 outline-none focus:border-[#E8620A] max-[932px]:rounded-xl max-[932px]:py-4 max-[932px]:text-lg" />
              <button type="button" onClick={buscarNumero} disabled={cargandoPagina} className="rounded-xl bg-[#1A1A1A] px-5 py-3 font-semibold text-white disabled:opacity-50">Buscar</button>
            </div>

            <div className="flex w-full gap-2 max-[932px]:flex-col">
              <input value={busquedaContiene} onChange={(e) => setBusquedaContiene(e.target.value.replace(/\D/g, "").slice(0, 4))} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); buscarNumeroQueContenga(); } }} placeholder="Buscar número que contenga... Ej: 26" className="w-full rounded-md border border-[#E0D9CE] bg-white px-4 py-2.5 outline-none focus:border-[#E8620A] max-[932px]:rounded-xl max-[932px]:py-4 max-[932px]:text-lg" />
      ...
      <section className="mx-auto mt-6 flex max-w-[1100px] flex-wrap gap-2 px-5 pb-8 max-[932px]:flex-col max-[932px]:px-3">
        <button type="button" disabled={pagina === 0 || cargandoPagina || enModoBusqueda} onClick={() => cambiarPagina(pagina - 1)} className="rounded-2xl border border-[#1A1A1A] bg-white px-5 py-4 text-lg font-semibold disabled:opacity-40">Ver números anteriores</button>
        <button type="button" disabled={pagina >= totalPaginas - 1 || cargandoPagina || enModoBusqueda} onClick={() => cambiarPagina(pagina + 1)} className="rounded-2xl bg-[#E8620A] px-5 py-4 text-lg font-semibold text-white disabled:opacity-40">Ver más números</button>
      ...
      {modalAbierto && (
        <div className="fixed inset-0 z-[500] flex items-center justify-center bg-black/55 p-4 max-[932px]:items-end max-[932px]:p-0">
          <div className="relative h-[90vh] max-h-[620px] w-full max-w-[520px] overflow-hidden rounded-xl bg-white max-[932px]:h-[94dvh] max-[932px]:max-h-[94dvh] max-[932px]:max-w-none max-[932px]:rounded-t-3xl max-[932px]:rounded-b-none">
            <div className="flex h-[52px] items-center justify-between bg-[#1A1A1A] px-4 text-lg font-semibold text-white max-[932px]:h-16 max-[932px]:px-5 max-[932px]:text-[22px]"><span>Confirmar reserva</span><button type="button" onClick={() => setModalAbierto(false)} className="text-3xl leading-none text-white max-[932px]:text-[34px]">×</button></div>
            <div className="h-[calc(100%-52px)] max-[932px]:h-[calc(100%-64px)]"><iframe src={formularioUrl} className="h-full w-full border-0" title="Formulario de compra" /></div>
          </div>
        </div>
      )}
