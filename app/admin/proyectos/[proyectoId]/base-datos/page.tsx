    ...
    <main className="min-h-screen bg-[#F2EDE4] p-4 text-[#1A1A1A]">
      <section className="mx-auto max-w-[1600px]">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-sm uppercase tracking-[3px] text-[#7A7066]">Base de datos</p>
        ...
        <div className="mb-5 grid gap-3 rounded-2xl bg-white p-4 md:grid-cols-3 xl:grid-cols-6">
          ...
          <input value={vendedor} onChange={(e) => setVendedor(e.target.value)} placeholder="Vendedor" className="rounded-xl border p-3" />
                ...
                {filtradas.map((b) => {
                  ...
                  return (
                    <tr key={b.id} className="border-b align-top">
                      ...
                      <td className="p-2"><input value={valor.vendedor_nombre || ""} onChange={(e) => editar(b.id, "vendedor_nombre", e.target.value)} className="w-44 rounded border p-2" /></td>
                      <td className="p-2"><select value={valor.canal || ""} onChange={(e) => editar(b.id, "canal", e.target.value)} className="w-36 rounded border p-2"><option value="">Sin canal</option>{CANALES.filter(Boolean).map((item) => <option key={item}>{item}</option>)}</select></td>
